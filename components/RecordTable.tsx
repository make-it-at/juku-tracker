...
export default function RecordTable({ records }: Props) {
  ...
  return (
    <div className="bg-gray-800 rounded-2xl p-5 shadow-lg overflow-x-auto">
      <h3 className="text-white text-sm font-medium opacity-80 mb-3">通塾記録</h3>
      <table className="w-full text-sm">
        <thead>
          <tr className="text-gray-400 border-b border-gray-700">
            <th className="text-left py-2 pr-3">日付</th>
            <th className="text-left py-2 pr-3">入室</th>
            <th className="text-left py-2 pr-3">退室</th>
        ...
        <tbody>
          {records.map((r) => (
            <tr key={r.id} className="border-b border-gray-700/50 text-gray-200 hover:bg-gray-700/30">
              ...
              <td className="py-2 pr-3">{r.checkOut || '-'}</td>
              <td className="py-2 text-right text-blue-400 font-mono">{r.durationH || `${r.durationMin}分`}</td>
            </tr>
          ))}
        </tbody>
      </table>
